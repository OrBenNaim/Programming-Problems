...
def find_leaders(arr):
    ...
    check_leader(arr, arr[n-1], n-2)
    print(arr[n-1])

def check_leader(arr, maxNum, i):   # recursive function
    if i == -1:
        return
    
    if arr[i] > maxNum:
        check_leader(arr, arr[i], i-1)  # Recursive call before printing leader
        print(arr[i], end=" ")          # Print leader with a space after
    
    else:
        check_leader(arr, maxNum, i-1)
...
#     #---------------------- Test Leaders in an array Problem ----------------------
